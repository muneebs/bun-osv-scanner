...
describe('isResolvable', () => {
  test.each([
    ...
    'v1.2.3',
    '10.0.0-rc.1',
  ])('accepts "%s"', (v) => expect(isResolvable(v)).toBe(true));
...
describe('batchQuery', () => {
  ...
  test('sends packages as OSV querybatch payload with npm ecosystem', async () => {
    ...
    expect(JSON.parse(options.body as string)).toEqual({
      queries: [
        { version: '4.17.4', package: { name: 'lodash', ecosystem: 'npm' } },
      ],
    });
  });
  ...
  test('throws on non-OK response', async () => {
    ...
    await expect(batchQuery([pkg('x', '1.0.0')])).rejects.toThrow(
      'OSV API 400'
    );
  });
  ...
  test('splits packages into chunks of 1000', async () => {
    ...
    fetchSpy
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ results: Array(1000).fill({ vulns: [] }) })
        )
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ results: [{ vulns: [] }] }))
